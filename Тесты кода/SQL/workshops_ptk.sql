...
--
-- Хост: 127.0.0.1:3307
-- Время создания: Мар 03 2021 г., 15:44
...
SELECT IF ( EXISTS(SELECT * FROM users WHERE ((users.login = login) OR (users.email = login)) AND (users.password = PASSWORD(SHA(pass)))), true, false) AS 'rezult'$$

CREATE DEFINER=`root`@`%` PROCEDURE `check_activity_on_workshop` (IN `ID` SMALLINT, IN `date_Start` DATE, IN `time_ID` SMALLINT)  NO SQL
SELECT IF (EXISTS(SELECT * FROM activity
LEFT OUTER JOIN workshops ON workshops.workshopsID = activity.workshop
LEFT OUTER JOIN time_intervals ON activity.time_interval = time_intervals.timeintervalID
WHERE (activity.workshop = ID) AND (activity.date = date_Start) AND activity.time_interval = time_ID), false, true) AS 'rezult'$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `get_activity` (IN `start_Date` DATE, IN `end_Date` DATE)  NO SQL
SELECT
...
WHERE (activity.date >= start_Date) AND (activity.date <= end_Date)$$

CREATE DEFINER=`root`@`%` PROCEDURE `get_my_activity` (IN `user_ID` SMALLINT, IN `start_Date` DATE, IN `end_Date` DATE)  NO SQL
SELECT
activity.activityID AS 'ID', activity.title AS 'title', activity.date AS 'date', time_intervals.title AS 'time_title', time_intervals.time_start AS 'time_start', time_intervals.time_end AS 'time_end', users.lastname AS 'lastname', users.name AS 'name', users.middlename AS 'midelename', workshops.kabinet AS 'kabinet', activity.description AS 'description', type_activity.title AS 'type_title'
FROM activity
LEFT OUTER JOIN time_intervals ON activity.time_interval = time_intervals.timeintervalID
...
LEFT OUTER JOIN workshops ON activity.workshop = workshops.workshopsID
LEFT OUTER JOIN users ON activity.owner = users.userID
WHERE (activity.date >= start_Date) AND (activity.date <= end_Date) AND (activity.owner = user_ID)$$
...
CREATE DEFINER=`root`@`%` PROCEDURE `get_workshop` (IN `ID` SMALLINT)  NO SQL
SELECT
workshops.workshopsID AS 'workshopID', workshops.title AS 'title', workshops.description AS 'description', workshops.kabinet AS 'kabinet', users.lastname AS 'lastname', users.name AS 'name', users.middlename AS 'middlename'
FROM workshops
LEFT OUTER JOIN users ON workshops.owner = users.userID
WHERE workshops.workshopsID = ID$$

CREATE DEFINER=`root`@`%` PROCEDURE `get_workshops` ()  NO SQL
SELECT
workshops.workshopsID AS 'workshopID', workshops.title AS 'title', workshops.description AS 'description', workshops.kabinet AS 'kabinet', users.lastname AS 'lastname', users.name AS 'name', users.middlename AS 'middlename'
FROM workshops
LEFT OUTER JOIN users ON workshops.owner = users.userID$$
